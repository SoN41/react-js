...
const CarDetails = ({ isOpen , closeModel , car } : CarDetailsPrps) => {
  return (
    <div>
      <>
        <Transition appear show={isOpen} as={Fragment}>
          <Dialog as='div' className='relative' onClose={closeModel}>
            <Transition.Child
              as={Fragment}
            ...
            <div className='fixed inset-0 overflow-y-auto'>
              ...
              >
                  ...
                  transform rounded-2xl bg-white p-6 text-left shadow-xl transition-all flex flex-col gap-5'>
                    <button
                      type='button'
                      className='absolute top-2 z-10 w-fit gap p-2 bg-primary-blue-100 rounded-full'
                      onClick={closeModel}
                    >
                      <Image 
                        src='/close.svg'
                      ...
                      <div className='mt-3 flex flex-wrap gap-4'>
                        {Object.entries(car).map(([key,value]) => (
                          <div className='flex justify-between gap-5 w-full text-right' key={key}>
                            ...
                            <p text-black-100 font-semibold >{value}</p>
                          </div>
                        ))}
                      </div>
                    </div>
